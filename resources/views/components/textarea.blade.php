...
@else
    <div {{ $attributes->merge(['class' => 'mb-4 flex items-start flex-col']) }}>
        ...
        <textarea class="h-[130px] border border-gray-300 py-1 px-2 rounded-md outline-none focus:border-sky-500 w-full @error(dot_name($name)) border-red-500 @enderror"
            type="text" 
            name="{{ $name }}" 
            id="{{ $id }}" 
